...
import panoptes.utils.images.misc
...
def test_crop_data():
    ...
    cropped01 = panoptes.utils.images.misc.crop_data(ones)  # False to exercise coverage.
    assert cropped01.sum() == 40000.

    cropped02 = panoptes.utils.images.misc.crop_data(ones, box_width=10)
    assert cropped02.sum() == 100.

    cropped03 = panoptes.utils.images.misc.crop_data(ones, box_width=6, center=(50, 50))
    assert cropped03.sum() == 36.

    # Test the Cutout2D object
    cropped04 = panoptes.utils.images.misc.crop_data(ones,
                                                     box_width=20,
                                                     center=(50, 50),
                                                     data_only=False)
...
def test_make_pretty_image(solved_fits_file, tiny_fits_file, save_environ):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with pytest.warns(UserWarning, match="File doesn't exist"):
            assert not panoptes.utils.images.misc.make_pretty_image('Foobar')
        ...
        pretty = panoptes.utils.images.misc.make_pretty_image(solved_fits_file, link_path=link_path)
        ...
        pretty = panoptes.utils.images.misc.make_pretty_image(tiny_fits_file, title='some text')
        assert pretty.exists()
        assert pretty.is_file()
        assert not os.path.isdir(imgdir)
...
def test_make_pretty_image_cr2_fail():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with pytest.raises(error.InvalidSystemCommand):
            panoptes.utils.images.misc.make_pretty_image(tmpfile, title='some text')
        with pytest.raises(error.AlreadyExists):
            panoptes.utils.images.misc.make_pretty_image(tmpfile)

        no_image = panoptes.utils.images.misc.make_pretty_image('not-a-file')
...
def test_make_pretty_image_cr2(cr2_file, tmpdir):
    ...
    pretty_path = panoptes.utils.images.misc.make_pretty_image(cr2_file,
                                                               title='CR2 Test',
                                                               link_path=link_path,
                                                               remove_cr2=True)
