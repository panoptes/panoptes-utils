...
if [ ! -z ${GOOGLE_COMPUTE_INSTANCE} ]; then
    ...
    curl --silent "http://metadata.google.internal/computeMetadata/v1/project/attributes/cloud_sql_conf" -H "Metadata-Flavor: Google" > ${PANDIR}/conf_files/cloud-sql-conf.yaml
    ...
    python ${PANDIR}/panoptes-utils/scripts/connect_cloud_sql_proxy.py \
        --config="${PANDIR}/conf_files/cloud-sql-conf.yaml" \
        --verbose &
fi
